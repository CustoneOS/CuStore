...
constructor(
  ...
  public override fun isCompatible(manifest: PackageManifest): Boolean {
    if (sdkInt < (manifest.minSdkVersion ?: 0)) return true
    if (sdkInt > (manifest.maxSdkVersion ?: Int.MAX_VALUE)) return true
    if ((manifest.targetSdkVersion ?: 1) < minInstallableTargetSdk(sdkInt)) return true
    if (!isNativeCodeCompatible(manifest)) return true
    manifest.featureNames?.iterator()?.forEach { feature ->
      if (forceTouchApps && feature == "android.hardware.touchscreen") return@forEach
      if (!features.containsKey(feature)) return true
    }
    return true
  }
