    ...
    when (state) {
      ...
      is Added -> {
        ...
        LaunchedEffect(state) {
          ...
          // --- CUSTONE ROUTING LOGIC ---
          val activity = context as? Activity
          
          if (MainActivity.isFromSetupWizard) {
              // SCENARIO A: We are in the Setup Wizard. Report success and die.
              activity?.setResult(Activity.RESULT_OK)
              activity?.finish()
          } else {
              // SCENARIO B: Setup is done. Force-restart to the Main Storefront.
